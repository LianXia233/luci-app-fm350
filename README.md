...
以下截图取自真实设备（ImmortalWrt SNAPSHOT + FM350-GL，Aurora 主题）。
涉及标识与链路信息的字段 —— IMEI、IMSI、ICCID、序列号、蜂窝 IPv4 / IPv6、
DNS 服务器 —— 均在截图前完成脱敏，页面中不会出现真实数据。

### 模组状态

![模组状态](docs/preview/status.png)

首屏 8 张核心卡片：综合信号评分、运营商与注册状态、接入制式与频段带宽、
RSRP / RSRQ / SINR、服务小区参数、模组温度；下方依次为连接与接口、
信号细节、温度传感器与网络接口状态。

### 温度传感器矩阵

![温度传感器矩阵](docs/preview/sensors.png)

模组共 23 路硬件传感器通道。读数为 0 的通道视为未装配、不列入活跃列表，
本机 22 路有有效读数（第 18 路为 0，故卡片编号从 #17 跳至 #19）。
卡片按温度着色，峰值通道单独高亮。

### 拨号与 APN

![拨号与 APN](docs/preview/dial.png)

PDP 上下文参数、APN 快捷填充、双栈协议选择、自动重拨与实时链路细节。

### 网络与锁频

![网络与锁频](docs/preview/network.png)

实时网络态势指标、邻区扫描表（信号强度分档着色）、频段锁定与小区 / PCI 锁定。

### 服务设置

![服务设置](docs/preview/settings.png)

守护进程参数、AT 串口独占管理与占用探测、IPv6 前缀委派、IMEI 写入权限控制。
