...
截图取自真实设备（ImmortalWrt SNAPSHOT + FM350-GL，Aurora 主题）。每个页面自上而下分段截取，
**覆盖整页内容**；分段位置都落在一整行没有文字的空行上，所以不会把文字截断。

脱敏说明：涉及身份、链路与基站位置的字段，一律在截图**之前**替换为 `[已脱敏]`，真实值不会进入图片——

| 类别 | 字段 |
| :--- | :--- |
| 身份与卡信息 | IMEI、序列号、IMSI、ICCID、短信中心号码 |
| 链路信息 | 蜂窝 IPv4 / IPv6 地址、DNS 服务器 |
| 基站信息 | 基站标识 ECI/NCI（小区 ID）、LAC / TAC、物理基站与物理小区 PCI、频点 ARFCN（含下行载波频点） |

保留的是公开的频谱与信号质量指标（接入制式、频段号如 `n41`、带宽档位、RSRP / RSRQ / SINR、
模组温度）与设备节点（如 `/dev/ttyUSB1`）。它们不含位置信息，遮掉反而会让截图失去说明价值。

### 模组状态

![模组状态 · 1/4](docs/preview/status-1.png)

标题头（固件版本、在线状态、刷新）与「网络概览」8 张核心卡片：综合信号评分、运营商与注册状态、
接入制式与频段带宽、RSRP、RSRQ、SINR、服务小区参数、模组温度；下方开始「连接与接口」。

![模组状态 · 2/4](docs/preview/status-2.png)

「连接与接口」余下卡片与「信号细节」全表——PCI、ARFCN、LAC / TAC、小区 ID 均已脱敏，
频段、带宽档位与各项信号质量保留；再往下是温度传感器矩阵的前 12 路。

![模组状态 · 3/4](docs/preview/status-3.png)

温度传感器矩阵余下通道与「识别信息」表（IMEI、序列号、IMSI、ICCID、短信中心全部脱敏）。
模组共 23 路硬件传感器通道，读数为 0 的通道视为未装配、不列入活跃列表——本机 22 路有有效读数，
第 18 路为 0，故卡片编号从 #17 跳到 #19；峰值通道单独标记 PEAK。

![模组状态 · 4/4](docs/preview/status-4.png)

「网络接口」表（接口名、物理网卡、地址与接口路由）、页面底部的口径说明，以及主题页脚。

### 拨号与 APN

![拨号与 APN · 1/2](docs/preview/dial-1.png)

PDP 无线链路状态、PDP 上下文配置（APN、常用运营商快捷填选、双栈协议选择）、
链路运行细节（连接状态、地址、DNS、重拨计数）。
...
「实时网络态势与射频指标」（物理基站 PCI 与下行载波频点已脱敏）、
「周边基站感知与邻区扫描」（信号强度分档着色的邻区表，频点 / 物理小区 PCI / 基站标识 ECI-NCI
三列已脱敏）、「工作频段限定与锁定（Band Lock）」及其 5G NR 频段矩阵。

![网络与锁频 · 2/2](docs/preview/network-2.png)

「物理小区锁定（Cell Lock / PCI）」与「蜂窝制式首选项（RAT Mode）」。

### 短信

![短信](docs/preview/sms-1.png)

短消息管理：存储位置与容量、发送短信（收件人、正文、超长短信自动拆分）、收件箱列表与刷新。

### AT 终端

![AT 终端](docs/preview/at-1.png)

AT 交互终端：安全沙箱提示、命令输入与回显、常用 AT 指令快捷库（四类分组，点击即填入）、
TTY 会话窗口与历史翻阅。
...
| **网络类型** | `5G NR` | 频段名与带宽（如 `n41` / `100 MHz`；模组上报的原始拼接编码由后端解码） |
...
> 页面下方依次按流式卡片呈现：**连接与接口** $\rightarrow$ **信号细节** $\rightarrow$ **温度传感器（共 23 路通道）** $\rightarrow$ **模组识别信息** $\rightarrow$ **网络接口状态**。
...
| :--- | :--- | :--- |
| **状态** | `/cgi-bin/luci/admin/modem/fm350/status` | 8 张仪表盘卡片、频段详情、温度传感器明细（最多 23 路） |
...
- [ ] **温度与传感器正常**：首屏显示基带实测温度（如 `45.5 ℃`），传感器明细表完整展开全部**有效读数**通道（模组共 23 路通道，本机实测 22 路读数非零，第 18 路读数为 0、已按「未装配」跳过），不包含 `[object Object]` 或单一异常的 `1`。
...
3. **`AT+GTACT` 的频段编号有三套编码**：
   ...
   * **服务小区的 `<band>` 与 `<bandwidth>` 也是编码而非人可读名**（`AT+GTCCINFO?` 同一套拼接规则）。
     实机 NR 服务小区返回 `...,504990,128,5041,500,...`：`5041` 应显示为 `n41`，
     带宽编码 `500` 按「值 ÷ 5 = MHz」换算为 `100 MHz`（手册 p191）；
     LTE 下该字段则是资源块（RB）数（`6` = 1.4 MHz …… `100` = 20 MHz）。
     后端按 `<rat>` 选表解码，编码落在表外时保留原始码并标注「编码未识别」。
4. **频段缺失与 ARFCN 动态反推**：
...
* **正确做法**：必须下发带参数的执行指令 `AT+GTSENRDTEMP=0`，模组将逐行返回 23 路传感器的原始千分位读数（单位： $0.001\text{ ℃}$，实际读数需除以 1000）。**这 23 行是通道总数，不等于有效读数数**：读数为 0 的通道表示该路未装配，解析器会跳过（本机实测第 18 路读数为 0，故活跃列表为 22 路）。常用传感器：`1` = soc_max，`10` = md_5g，`14` = ltepa_ntc，`15` = nrpa_ntc，`16` = rf_ntc。
</details>
