...
└──────────────────────────────┬──────────────────────────────┘
                               │  uhttpd / ubus JSON-RPC（rpcd 已注册方法）
                               ▼
┌─────────────────────────────────────────────────────────────┐
│           rpcd 代理插件 (/usr/share/rpcd/ucode/fm350.uc)    │
│            - 参数校验 + 单引号指令转义                       │
│            - 以绝对路径 /usr/sbin/fm350d <cmd> 桥接后端      │
└──────────────────────────────┬──────────────────────────────┘
                               │  exec
                               ▼
┌─────────────────────────────────────────────────────────────┐
│            fm350d —— Rust 单二进制（CLI 与 daemon 同体）     │
│  ┌────────────────────────────────────────────────────────┐ │
│  │ 本地 JSON API（api/，仅 127.0.0.1:8766）                │ │
│  │  /api/ports · /api/at · /api/eif · /api/status …        │ │
│  │  CLI 命令 daemon 转发优先；daemon 未运行/无响应时        │ │
│  │  本地直连 AT 口（见下方虚线分支）                        │ │
│  └────────────────────────────────────────────────────────┘ │
│  - AT 口持久独占：exclusive(true) + ioctl(TIOCEXCL) + flock  │
│  - 半双工 AT 互斥队列调度；URC 行门截流（+GT* → 事件队列）   │
│  - 网络守护：netifd/UCI 接口骨架、默认路由自愈、连通性保活   │
│  - /proc/*/fd 巡检：发现外部进程绕过锁打开 AT 口即告警       │
└──────────────┬──────────────────────────────────┬───────────┘
               │ 控制面：/dev/ttyUSBx               │ 数据面：RNDIS 网卡
               │ （独占持有，全运行期不释放）        │ （netifd 接口 fm350 / fm350v6）
               ▼                                   ▼
┌─────────────────────────────────────────────────────────────┐
│                   Fibocom FM350-GL 硬件模组                  │
│   AT 命令 / URC / +GT* 事件：USB 串口（ttyUSB*）             │
│   蜂窝数据：RNDIS 通道（无 DHCP，IPv4/6 由守护从             │
│   AT+CGPADDR 提取并下发到接口）                              │
└─────────────────────────────────────────────────────────────┘
        ▲  可选：+GT* URC 注入 AT 读取路径（1.0.12 起，虚线）
┌─────────────────────────────────────────────────────────────┐
│   f22-atproxy（可选增强，随修改版 rootfs 提供，订阅模组       │
│   EIF_IND 并降维输出 +GT* URC；未刷入则本分支不存在）        │
└─────────────────────────────────────────────────────────────┘
```

> [!NOTE]
> **控制面与数据面分离**：AT 口（`/dev/ttyUSB*`）只承载 AT 命令、URC 与 EIF 事件，
> 蜂窝数据全部走 RNDIS 网卡。守护通过 UCI 接口骨架（`fm350` / `fm350v6`）把模组侧
> 地址（`AT+CGPADDR`）应用到主机侧，并周期性巡检默认路由与连通性（详见
> 「AT 串口独占与主动可观测机制」与「拨号链路判据」章节）。
>
> **CLI 双路径**：`fm350d <cmd>` 优先经 `127.0.0.1:8766` 转发给常驻 daemon 执行；
> daemon 未运行或无响应时（如 `ports` 命令 2 秒短超时后），CLI 在自身进程内
> 独占打开 AT 口、执行后释放 —— 这是「daemon 拉不起来也能排障」的兜底路径。
...
### AT 口自动改选（哑口自愈）与 LuCI 卡死四层防御

FM350 在 RNDIS+AT 模式下导出 7 个 ttyUSB（DIAG / GNSS / AT / MODEM / log 等），
**只有真正的 AT 口才应答命令**。USB 重枚举或模组重启后 tty 编号会漂移，配置里的
`at_port` 可能变成一个「节点存在但永不说话」的哑口——每条 AT 命令等满
`at_timeout`，一轮巡检数分钟，后端表现为完全瘫痪。1.0.13 起内置四层防御：

| 层 | 机制 | 效果 |
| :--- | :--- | :--- |
| 1 | 哑口自动改选（`AtHandle`） | 连续 3 条 AT 命令零字节应答即判定端口失联，自动探测仍应答 `AT` 的 Fibocom 候选口并写回 UCI（与「路径消失改选」同一机制）；探测失败冷却 60 s |
| 2 | 锁获取上限 3 s（`AtHandle::with`） | API 请求限时拿锁，超时返回「AT 会话忙」明确错误，不再无限排队 |
| 3 | CLI 读类转发超时 15 s（`cli.rs`） | daemon 无响应时快速回落本地直连，给出明确错误 |
| 4 | rpcd 兜底 `timeout`（`fm350.uc`，读 20 s / 写 90 s） | 后端无论怎么卡，rpcd 单次调用最多占用该时长，Web 管理界面绝不被拖死 |

rpcd 的 ucode 插件在 rpcd 主循环内同步执行 CLI——后端堵多久，LuCI 登录、
session、uci 等全部 ubus 调用就堵多久（表现为整个 Web 界面瘫痪）。第 4 层是
最终兜底：任何后端故障都只表现为插件页面短暂显示错误，管理界面本体永远可用。
...
| `net_guard_rounds` | `3` | 连续多少轮连通性探测失败才触发该栈恢复动作（每栈冷却 300 s）；同一门槛也用于连续拨号失败/第 2 级重拨失败后的会话地址净化，净化后自动拨号退避 300 s |
...
- **主机侧静态地址残值**：`network.fm350` 的骨架为 `proto=static`，上一次拨号成功写入的 `ipaddr/gateway/dns` 会被 netifd 开机照常应用，和 SIM/PDP 状态无关。自动拨号连续失败达到 `net_guard_rounds`（默认 3）后，守护会 `ifdown` 主/IPv6 接口并删除两栈会话地址、DNS 与插件生成路由段，提交 UCI 但保留 `proto/device/auto/metric` 骨架；随后自动重拨退避 300 s。`net_guard` 的第 2 级重拨连续失败达到同一门槛时也触发净化。状态/拨号页的会话 IP/DNS 直接取自模组 PDP 字段，并仅在 PDP 激活时展示；状态页不再把主机内核静态残值当作当前会话地址。
