...
#
# 目标平台：ImmortalWrt SNAPSHOT / mediatek-filogic / aarch64_cortex-a53
# 产物：dist-release/luci-app-fm350-<版本>.apk（若该 SDK 同时产出 ipk 则一并收集）
...
#   2. 断言只在 .pkgdir 暂存目录上做，不去解最终的 .apk。OpenWrt 25.x 起
...
sdk_base="${FM350_SDK_BASE:-https://downloads.immortalwrt.org/snapshots/targets/mediatek/filogic}"
...
expected_apk="${pkg_name}-${pkg_version}-r${pkg_release}.apk"

step "构建 ${pkg_name} ${pkg_version}-r${pkg_release}（期望产物 ${expected_apk}）"
...
step "把源码放进 SDK 的 package/${pkg_name}"
...
step "更新 feeds（首次克隆，约数分钟）"
./scripts/feeds update -a
./scripts/feeds install luci-base

step "写入最小 .config 并 defconfig"
cat > .config <<EOF
...
step "编译（日志：${work_dir}/build.log）"
if ! make "package/${pkg_name}/compile" -j"$(nproc)" V=s >"${work_dir}/build.log" 2>&1; then
	printf -- '---- 构建失败，日志中的错误行 ----\n' >&2
	grep -inE 'error(\[|:)|No rule to make|cannot find|undefined reference|failed to' "${work_dir}/build.log" \
		| tail -n 30 >&2 || true
	printf -- '---- 日志尾部 40 行 ----\n' >&2
	tail -n 40 "${work_dir}/build.log" >&2
	fail "make package/${pkg_name}/compile 失败，完整日志：${work_dir}/build.log"
fi
...
step "断言 1/4：产物存在且版本号与 Makefile 一致"
found="$(find bin -type f -name "${expected_apk}" | head -n 1 || true)"
[ -n "${found}" ] || {
	printf -- '---- bin 下的 fm350 产物 ----\n' >&2
	find bin -type f -name "${pkg_name}*" >&2 || true
	fail "未找到期望产物 ${expected_apk}（版本号与 Makefile 不一致？）"
}
...
note "${found}"
note "收集到 ${artifact_count} 个包产物"

step "断言 2/4：进包的 fm350d 是 aarch64 可执行文件"
pkg_roots="$(find build_dir -type d -path "*${pkg_name}-*/.pkgdir/${pkg_name}" 2>/dev/null || true)"
[ -n "${pkg_roots}" ] || fail "未找到 .pkgdir 暂存目录，无法校验进包内容"
pkg_root_count=0
for root in ${pkg_roots}; do
	pkg_root_count=$((pkg_root_count + 1))
	[ -f "${root}/usr/sbin/fm350d" ] || fail "包内缺少 /usr/sbin/fm350d（${root}）"
	[ -s "${root}/usr/sbin/fm350d" ] || fail "包内 fm350d 大小为 0（${root}）"
	file "${root}/usr/sbin/fm350d" | grep -q 'aarch64' \
		|| fail "包内 fm350d 不是 aarch64 可执行文件：$(file -b "${root}/usr/sbin/fm350d")"
	note "$(file -b "${root}/usr/sbin/fm350d")"
done

step "断言 3/4：进包的文本文件与 git 源码逐字节一致"
pairs=""
...
while IFS='|' read -r src_rel packed_rel; do
	...
	for root in ${pkg_roots}; do
		...
		[ -f "${packed}" ] || fail "未打进包：${packed_rel}（${root}）"
		cmp -s "${src}" "${packed}" || fail "${packed_rel} 与源码不一致（包内 $(wc -c <"${packed}") 字节 / 源码 $(wc -c <"${src}") 字节）"
		checked=$((checked + 1))
	done
done <<EOF
${pairs}
EOF
[ "${checked}" -gt 0 ] || fail "没有任何文件被比对，断言本身失效"
...
if [ -f "${sdk_dir}/public-key.pem" ]; then
	...
	note "SDK 未提供 public-key.pem，本次不附带公钥"
fi
